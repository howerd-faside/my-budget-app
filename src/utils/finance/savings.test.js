...
import { affordabilityStatus, affordabilityDate, findGoalHit, calcPortfolioStats, enrichHoldings } from './savings';
...
describe('calcPortfolioStats', () => {
  const holdings = [
    { units: '10', avgCost: '100', currentPrice: '120', category: 'NZ Shares' },
    { units: '5',  avgCost: '200', currentPrice: '180', category: 'US Shares' },
  ];

  it('calculates totalValue correctly', () => {
    // 10 * 120 + 5 * 180 = 1200 + 900 = 2100
    expect(calcPortfolioStats(holdings, [], []).totalValue).toBeCloseTo(2100, 4);
  });

  it('calculates totalCost correctly', () => {
    // 10 * 100 + 5 * 200 = 1000 + 1000 = 2000
    expect(calcPortfolioStats(holdings, [], []).totalCost).toBeCloseTo(2000, 4);
  });

  it('calculates unrealised gain/loss', () => {
    // 2100 - 2000 = 100
    expect(calcPortfolioStats(holdings, [], []).unrealised).toBeCloseTo(100, 4);
  });

  it('calculates returnPct', () => {
    // 100 / 2000 * 100 = 5%
    expect(calcPortfolioStats(holdings, [], []).returnPct).toBeCloseTo(5, 4);
  });

  it('sums contributions', () => {
    const contribs = [{ amount: '500' }, { amount: '300' }];
    expect(calcPortfolioStats(holdings, contribs, []).totalContrib).toBeCloseTo(800, 4);
  });

  it('sums net dividends', () => {
    const dividends = [{ netAmount: '100' }, { netAmount: '50.50' }];
    expect(calcPortfolioStats(holdings, [], dividends).totalDivNet).toBeCloseTo(150.5, 4);
  });

  it('returns empty allocation for empty holdings', () => {
    const { allocation } = calcPortfolioStats([], [], []);
    expect(allocation).toEqual([]);
  });

  it('allocation percentages sum to 1 (or close)', () => {
    const { allocation } = calcPortfolioStats(holdings, [], []);
    const total = allocation.reduce((s, a) => s + a.pct, 0);
    expect(total).toBeCloseTo(1, 4);
  });

  it('allocation is sorted descending by value', () => {
    const { allocation } = calcPortfolioStats(holdings, [], []);
    for (let i = 1; i < allocation.length; i++) {
      expect(allocation[i].val).toBeLessThanOrEqual(allocation[i - 1].val);
    }
  });

  it('returnPct is 0 when totalCost is 0', () => {
    const zeroHoldings = [{ units: '0', avgCost: '0', currentPrice: '100', category: 'Cash' }];
    expect(calcPortfolioStats(zeroHoldings, [], []).returnPct).toBe(0);
  });

  it('handles string-coercible numbers in holdings', () => {
    const h = [{ units: '100', avgCost: '10.50', currentPrice: '12.25', category: 'ETF' }];
    const { totalValue } = calcPortfolioStats(h, [], []);
    expect(totalValue).toBeCloseTo(1225, 2);
  });

  it('handles empty inputs without error', () => {
    expect(() => calcPortfolioStats([], [], [])).not.toThrow();
    expect(() => calcPortfolioStats([], null, null)).not.toThrow();
  });
});

// ── enrichHoldings ────────────────────────────────────────────────────────────
describe('enrichHoldings', () => {
  it('adds value = units * currentPrice', () => {
    const result = enrichHoldings([{ units: '10', currentPrice: '15', avgCost: '12' }]);
    expect(result[0].value).toBeCloseTo(150, 4);
  });

  it('adds cost = units * avgCost', () => {
    const result = enrichHoldings([{ units: '10', currentPrice: '15', avgCost: '12' }]);
    expect(result[0].cost).toBeCloseTo(120, 4);
  });

  it('adds gl = value − cost', () => {
    const result = enrichHoldings([{ units: '10', currentPrice: '15', avgCost: '12' }]);
    expect(result[0].gl).toBeCloseTo(30, 4);
  });

  it('adds glPct = gl / cost * 100', () => {
    const result = enrichHoldings([{ units: '10', currentPrice: '15', avgCost: '12' }]);
    expect(result[0].glPct).toBeCloseTo(25, 4);
  });

  it('gl is negative when currentPrice < avgCost', () => {
    const result = enrichHoldings([{ units: '10', currentPrice: '8', avgCost: '12' }]);
    expect(result[0].gl).toBeLessThan(0);
  });

  it('glPct is 0 when avgCost is 0', () => {
    const result = enrichHoldings([{ units: '10', currentPrice: '15', avgCost: '0' }]);
    expect(result[0].glPct).toBe(0);
  });

  it('preserves all original holding properties', () => {
    const h = { units: '5', currentPrice: '20', avgCost: '18', name: 'Test ETF', ticker: 'TEST', id: 'abc' };
    const result = enrichHoldings([h]);
    expect(result[0].name).toBe('Test ETF');
    expect(result[0].ticker).toBe('TEST');
    expect(result[0].id).toBe('abc');
  });

  it('handles empty array', () => {
    expect(enrichHoldings([])).toEqual([]);
  });

  it('handles null/undefined input gracefully', () => {
    expect(enrichHoldings(null)).toEqual([]);
    expect(enrichHoldings(undefined)).toEqual([]);
  });

  it('processes multiple holdings independently', () => {
    const result = enrichHoldings([
      { units: '10', currentPrice: '10', avgCost: '8' },
      { units: '5',  currentPrice: '20', avgCost: '25' },
    ]);
    expect(result[0].gl).toBeCloseTo(20, 4);
    expect(result[1].gl).toBeCloseTo(-25, 4);
  });
});
