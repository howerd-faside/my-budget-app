    ...
    .array(
      z.object({
        units: z
          ...
          .pipe(z.number({ invalid_type_error: 'Enter units' }).positive('Units must be greater than 0')),
        costPerUnit: z
          .union([z.string(), z.number()])
          .transform(v => Number(v))
          .pipe(z.number({ invalid_type_error: 'Enter cost' }).min(0, 'Cost cannot be negative')),
      })
    )
